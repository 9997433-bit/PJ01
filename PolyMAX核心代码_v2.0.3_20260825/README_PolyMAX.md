...
采样带宽过窄（弧长过短）时，各阶基函数几乎线性相关，正规方程严重病态，求得的
极点频率和阻尼可信度低。出现该告警时应扩大识别频带（PolyFreqMin/Max），或改用
更贴近识别频带的采样/细化设置后复核稳定图。

## 窄带病态告警（NarrowBandWarning）

p-LSCF 在 z 域使用基函数 `z^k = e^{-jkωdt}`，其中 `dt = 1/(2.5·f_max)`。这些基函数在单位圆上
只覆盖识别频带对应的弧段，弧长为 `(ω_max − ω_min)·dt`（弧度）。

当用户通过 `PolyFreqMin/Max` 聚焦窄频段时，弧长可能远小于 1 rad，导致各阶基函数高度线性相关、
正规方程条件数爆炸（cond(R) 可达 10¹⁷ 量级），识别出的频率/阻尼严重失真，且 PolyMAX 标志性的
“干净稳定图”特性也会失效。

`PolymaxResult.NarrowBandWarning` 在弧长 < 0.5 rad 时置位；`BasisArcLength` 给出实际弧长值。
出现告警时应加宽识别频带后重新识别，或等待后续版本的频移求解路径。
